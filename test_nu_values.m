...
Nu_values = [1, 2, 3]; % Different values of Nu to test
N = 20;
lambda = 1;
y_zad = 1;
...
xlabel('k'); title('u');
u_legends = cell(1, length(Nu_values));

figure; hold on; grid on;
xlabel('k'); title('y, y_{zad}');
stairs(0:kk, [0 y_zad*ones(1, kk)], 'r--');

for idx = 1:length(Nu_values)
    Nu = Nu_values(idx);
